---
name: ui-polish
description: UI/UX design and polish for the Looneyz Predictions Soccer Stats dashboard (Next.js App Router). Use when the user asks to redesign, polish, refine, modernize, restyle, or critique the UI/UX, or wants visual hierarchy, motion, data viz, or component-level work on match cards, score blocks, odds rows, prediction cards, streak tables, summary bars, or the master-detail layout.
---

# UI Polish — Looneyz Predictions

This skill owns visual and interaction design for the Next.js + Tailwind dashboard. UI changes belong in `app/page.jsx`, with shared styles in `app/globals.css` only when a page-local Tailwind change is not enough. The active dashboard reads generated JSON from `match_data.json` / `public/data/match_data.json`. The legacy `index.html` dashboard and `DATA_SOCCER` splicer have been removed.
...
2. **Open the app in a browser** at `http://localhost:3001` after starting it with `npm.cmd run dev` or `npm.cmd run dev:fresh`. Look at the live render before editing; CSS changes that read fine in the diff often break alignment in the layout.
3. **Respect the data**: `match_data.json` is auto-generated by the Python pipeline. Do not hand-edit generated data unless the user explicitly asks for a one-off diagnostic patch.
...
Source of truth is the Tailwind class system already used in `app/page.jsx`, plus any tokens in `app/globals.css`. Keep additions consistent with the existing dashboard palette and component density.
...
- **Don't introduce a "favourite" outline on odds cards.** Odds boxes should show value and market context, not imply an automatic bet unless the data explicitly marks a value pick.
...
- **Theme changes must follow the active Next.js implementation.** Check `app/page.jsx`, `app/globals.css`, and `app/layout.jsx` before adding theme logic or tokens.
...
- **Don't add new framework dependencies.** This app already uses Next.js, React, Tailwind, and `lucide-react`; use those existing tools unless the user explicitly asks for more.
...
2. **Read the relevant component and class blocks** in `app/page.jsx` before editing.
3. **Edit only the targeted component/class logic** unless the requested polish requires a shared style in `app/globals.css`.
4. **Open the app in the browser and verify** — describe what you see; don't claim "looks great" without looking.
