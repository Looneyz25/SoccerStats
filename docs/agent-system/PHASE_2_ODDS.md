...
- After the core slate exists, use `npm.cmd run get:data:sportsbet` for a smaller Sportsbet-only odds/market refresh. Use `npm.cmd run get:data:bet365` only to merge approved cache files (`docs/agent-system/inputs/bet365_context.json` or `statshub_context.json`) into future unlocked matches; do not crawl bet365/StatsHub at high frequency.
