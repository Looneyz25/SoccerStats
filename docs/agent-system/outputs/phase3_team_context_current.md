# Phase 3 Team Context

Timezone: Australia/Adelaide
Source: SofaScore (smart-mimic session)

## Run Notes

| Item | Value |
| --- | --- |
| run_timestamp | 2026-05-20 05:55:45 ACST |
| timezone | Australia/Adelaide |
| source | SofaScore (smart-mimic session) |
| phase2_input_rows | 2 |
| phase2_ready_rows | 1 |
| phase3_total_rows | 2 |
| ready_for_phase_4 | 0 |
| team_unresolved | 1 |
| partial_form | 0 |
| source_blocked | 0 |
| upstream_blocked | 1 |
| next_action | SofaScore endpoints failed. Check session warmup, IP, or rerun later. |

## Source Health

| Endpoint | Calls | OK | Errors | Health | Notes |
| --- | --- | --- | --- | --- | --- |
| search | 6 | 0 | 2 | blocked | HTTP 403 |

## Form Summary

| Date | League | Home | Away | H last5 (W-D-L GF-GA) | A last5 (W-D-L GF-GA) | H Streaks | A Streaks | H2H | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-05-20 | Premier League | Chelsea | Tottenham | -- - | -- - |  |  |  (H/A/D) | team_unresolved |
| 2026-05-20 | Premier League | Bournemouth | Manchester City | -- - | -- - |  |  |  (H/A/D) | upstream_blocked |
