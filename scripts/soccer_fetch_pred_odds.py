#!/usr/bin/env python3
"""For each match, attach `odds` to each prediction (winner, BTTS, ou_goals, ou_cards)."""
...
def attach_pred_odds(m, market):
    ...
    # Winner — use sportsbet odds first, fallback to SofaScore Full time
    w = p.get("winner") or {}
    ...
    if og.get("pick") and og.get("odds") is None:
        ...
        ou_key = "Over" if og["pick"] == "Over" else "Under"
        v = (market.get(f"Match goals {line}") or {}).get(ou_key)
    ...
    if oc.get("pick") and oc.get("odds") is None:
        ...
        ou_key = "Over" if oc["pick"] == "Over" else "Under"
        v = (market.get(f"Cards in match {line}") or {}).get(ou_key) or any_line(market, "Cards in match", ou_key)
...
def main():
    ...
    for L in store["leagues"]:
        for m in L["matches"]:
            ...
            if not eid: continue
            p = m.get("predictions") or {}
            need = any(p.get(k) and p[k].get("pick") and p[k].get("odds") is None for k in ("winner","ou_goals","btts","ou_cards"))
