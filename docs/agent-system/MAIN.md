...
- Keep the top-competition league shard first: UEFA Champions League, UEFA Europa League, Premier League, LaLiga, Bundesliga, Serie A, Ligue 1, UEFA Conference League, Brasileirão Betano, CONMEBOL Libertadores, A-League Men, FIFA World Cup, and International Friendly Games should be collected, sorted, and uploaded ahead of long-tail leagues.
- For speed, prefer split runs over one monolithic loop: result-only checks run from the due-time queue, full fixture discovery runs on the 7-day window, and deeper odds/context enrichment can run after the core slate is uploaded. Use `npm.cmd run get:data:sportsbet` for Sportsbet odds/market refreshes and `npm.cmd run get:data:bet365` only for cache-first bet365/StatsHub context merges.
